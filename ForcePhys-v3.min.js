...
(function(global) {
    ...
    ForcePhys.World = class {
        constructor({ canvasId, gravity = 0.5, worldBounds = true }) {
            this.gravity = gravity;
            this.objects = [];
            this.canvas = document.getElementById(canvasId);
            this.ctx = this.canvas.getContext('2d');
            this.running = false;
            this.worldBounds = worldBounds; // new option
        }
        ...
        update() {
            for (let obj of this.objects) {
                ...
                if (this.worldBounds) {
                    if (obj.type === 'circle') {
                        // circle bounds
                        if (obj.y + obj.r > this.canvas.height) {
                            obj.y = this.canvas.height - obj.r;
                            obj.vy *= -0.7;
                        }
                        if (obj.y - obj.r < 0) {
                            obj.y = obj.r;
                        ...
                        if (obj.x + obj.r > this.canvas.width) {
                            obj.x = this.canvas.width - obj.r;
                            obj.vx *= -0.7;
                        }
                        if (obj.x - obj.r < 0) {
                            obj.x = obj.r;
                        ...
                        // rect bounds
                        if (obj.y + obj.h > this.canvas.height) {
                        ...
                        if (obj.x + obj.w > this.canvas.width) {
                            obj.x = this.canvas.width - obj.w;
                            obj.vx *= -0.7;
                        }
                        if (obj.x < 0) {
                            obj.x = 0;
        ...
        draw() {
            ...
            for (let obj of this.objects) {
                ...
                if (obj.border) { 
                    this.ctx.strokeStyle = obj.border.color; 
                    this.ctx.lineWidth = obj.border.width; 
                }
                if (obj.type === 'circle') {
                    this.ctx.beginPath();
