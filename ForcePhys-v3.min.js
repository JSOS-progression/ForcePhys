/* ForcePhys v3.0 - single-file wrapper
   - Requires Matter.js already loaded or baked in
...
(function(global){
  ...
  if(!Matter) throw new Error("ForcePhys requires Matter.js to be loaded first.");
  ...
  ForcePhys.World = class {
    constructor({ canvasId, gravity=1, mouseDrag=true, debug=false, pixelRatio=1 } = {}) {
      ...
      Matter.Events.on(this.engine, 'beforeUpdate', e => this._updateKeyboardAndGravity(e));
    ...
    _updateKeyboardAndGravity(e){
      ...
      for(const id in this._keyboardBindings){
        ...
        if(!w) continue;
        const b = w.matter;
        if(this._pressed[cfg.left])  Matter.Body.applyForce(b, b.position, { x: -cfg.forceX*dt, y:0 });
        if(this._pressed[cfg.right]) Matter.Body.applyForce(b, b.position, { x:  cfg.forceX*dt, y:0 });
        if(this._pressed[cfg.up])    Matter.Body.applyForce(b, b.position, { x:0, y: -cfg.forceY*dt });
        if(this._pressed[cfg.down])  Matter.Body.applyForce(b, b.position, { x:0, y:  cfg.forceY*dt });
      }
      ...
      for(const w of this.bodies){
        ...
        if(gScale!==1){
          ...
          const extra = b.mass * worldG*(gScale-1)*dt;
          Matter.Body.applyForce(b, b.position, {x:0, y:extra});
        }
      }
    }
    ...
    add(spec){
      const wrapper = {};
      wrapper.type = spec.type || spec.constructor?.name.toLowerCase() || 'rect';
      wrapper.id = spec.id || uid('obj');
      wrapper.class = spec.class || spec.className || null;
      wrapper.color = spec.color || '#666';
      wrapper.border = spec.border || null;
      wrapper.bounciness = spec.bounciness ?? spec.restitution ?? 0;
      wrapper.friction = spec.friction ?? 0.1;
      wrapper.density = spec.density;
      wrapper.gravityScale = spec.gravityScale ?? 1;
      wrapper.draggable = spec.draggable ?? true;
      wrapper.static = !!spec.static;

      if(wrapper.type==='circle'){ wrapper.x=spec.x; wrapper.y=spec.y; wrapper.r=spec.r; }
      else if(wrapper.type==='rect'){ wrapper.x=spec.x; wrapper.y=spec.y; wrapper.w=spec.w; wrapper.h=spec.h; }
      else if(wrapper.type==='poly'){ wrapper.x=spec.x; wrapper.y=spec.y; wrapper.s=spec.s; wrapper.sides=spec.sides??3; }

      const opts = { restitution:wrapper.bounciness, friction:wrapper.friction, isStatic:wrapper.static };
      if(wrapper.density!==undefined) opts.density=wrapper.density;

      let body;
      if(wrapper.type==='circle') body = Matter.Bodies.circle(wrapper.x, wrapper.y, wrapper.r, opts);
      else if(wrapper.type==='rect') body = Matter.Bodies.rectangle(wrapper.x, wrapper.y, wrapper.w, wrapper.h, opts);
      else if(wrapper.type==='poly') body = Matter.Bodies.polygon(wrapper.x, wrapper.y, wrapper.sides, wrapper.s, opts);
      else throw new Error('ForcePhys: unknown type '+wrapper.type);

      body.forcePhys = wrapper;
      wrapper.matter = body;
    ...
    applyForce(id,fx,fy){ const w=this.getObjectById(id); if(!w) return; Matter.Body.applyForce(w.matter,w.matter.position,{x:fx,y:fy}); }
    setVelocity(id,vx,vy){ const w=this.getObjectById(id); if(!w) return; Matter.Body.setVelocity(w.matter,{x:vx,y:vy}); }
    getVelocity(id){ const w=this.getObjectById(id); if(!w) return null; return {x:w.matter.velocity.x, y:w.matter.velocity.y}; }

    enableKeyboardControl(id,cfg={}){ const def={ left:'ArrowLeft', right:'ArrowRight', up:'ArrowUp', down:'ArrowDown', forceX:0.002, forceY:0.004 }; this._keyboardBindings[id]=Object.assign({}, def, cfg); }
    disableKeyboardControl(id){ delete this._keyboardBindings[id]; }

    on(event, cb){ 
      if(event.toLowerCase().includes('start')) this._collisionListeners.start.push(cb); 
      else this._collisionListeners.end.push(cb); 
    }
    off(event, cb){
      const arr = (event.toLowerCase().includes('start')) ? this._collisionListeners.start : this._collisionListeners.end;
    ...
    _emitCollision(kind, ev){
      for(const pair of ev.pairs){
        ...
        const idA=a.forcePhys?.id||null, idB=b.forcePhys?.id||null;
        const contact={idA,idB,bodyA:a,bodyB:b,normal:pair.collision.normal,depth:pair.collision.depth};
        const listeners = kind==='start'? this._collisionListeners.start:this._collisionListeners.end;
        for(const cb of listeners) try{ cb(contact); } catch(e){ console.error(e); }
      }
    }
    ...
    _render(){
      ...
      for(const w of this.bodies){
        ...
        ctx.strokeStyle=(w.border?.color)||'#000';
        ctx.lineWidth=(w.border?.width)||0;
        if(b.circleRadius){ ctx.beginPath(); ctx.arc(0,0,b.circleRadius,0,Math.PI*2); ctx.fill(); if(w.border) ctx.stroke(); }
        else{ const verts=b.vertices; ctx.beginPath(); ctx.moveTo(verts[0].x-b.position.x,verts[0].y-b.position.y); for(let i=1;i<verts.length;i++) ctx.lineTo(verts[i].x-b.position.x,verts[i].y-b.position.y); ctx.closePath(); ctx.fill(); if(w.border) ctx.stroke(); }
        ctx.restore();
      }
    ...
    start(){ if(this._running) return; this._running=true; Matter.Engine.run(this.engine); this._raf=requestAnimationFrame(this._render); }
    stop(){ this._running=false; if(this._raf) cancelAnimationFrame(this._raf); }
  };

  ForcePhys.Circle = opts => Object.assign({}, opts, { type:'circle' });
  ForcePhys.Rect   = opts => Object.assign({}, opts, { type:'rect' });
  ForcePhys.Poly   = opts => Object.assign({}, opts, { type:'poly' });
