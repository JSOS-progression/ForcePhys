/* ForcePhys-v2.2 - Includes Matter.js internally */
...
// ---- World constructor ----
ForcePhys.World = function(options){
    this.canvas = document.getElementById(options.canvasId);
    if(!this.canvas) throw new Error('Canvas not found');
    this.ctx = this.canvas.getContext('2d');
    this.gravity = options.gravity || 0.5;
    this.mouseDrag = options.mouseDrag !== undefined ? options.mouseDrag : true;
    this.bodies = [];
    ...
    if(this.mouseDrag){
        ...
        // Override mouse drag per-body
        const originalStartDrag = this.mouseConstraint.startDrag;
        this.mouseConstraint.startDrag = (body) => {
            if(body && body.draggable === false) return false; // skip
            return originalStartDrag.call(this.mouseConstraint, body);
        };
    }
};

// ---- Add body to world ----
ForcePhys.World.prototype.add = function(body){
    // Apply bounciness
    if(body.bounciness !== undefined){
        body.options = body.options || {};
        body.options.restitution = body.bounciness;
    }

    // Store body in our list
    this.bodies.push(body);

    // Create Matter.js body
    let matterBody;
    if(body.type === 'circle'){
        matterBody = Matter.Bodies.circle(body.x, body.y, body.r, body.options || {});
    } else if(body.type === 'rect'){
        matterBody = Matter.Bodies.rectangle(body.x, body.y, body.w, body.h, body.options || {});
    } else if(body.type === 'poly'){
        matterBody = Matter.Bodies.polygon(body.x, body.y, body.sides, body.s, body.options || {});
    }
    matterBody.fpId = body.id;
    matterBody.draggable = body.draggable !== undefined ? body.draggable : true;
    body.matterBody = matterBody;

    Matter.World.add(this.engine.world, matterBody);
};

// ---- Start simulation ----
ForcePhys.World.prototype.start = function(){
    const ctx = this.ctx;
        ...
        bodies.forEach(b => {
            ...
            if(b.type === 'circle'){
                ...
                for(let i=1;i<vertices.length;i++){
                    ctx.lineTo(vertices[i].x-m.position.x, vertices[i].y-m.position.y);
                }
                ctx.closePath();
                ctx.fill();
            }
...
// ---- Simple body constructors ----
ForcePhys.Circle = function(options){ this.type='circle'; Object.assign(this, options); };
ForcePhys.Rect = function(options){ this.type='rect'; Object.assign(this, options); };
ForcePhys.Poly = function(options){ this.type='poly'; Object.assign(this, options); };

// ---- Style function (optional) ----
ForcePhys.Style = function(world, styles){
    // apply styles to bodies by id
    world.bodies.forEach(b=>{
        if(styles[b.id]){
            Object.assign(b, styles[b.id]);
        }
    });
};
